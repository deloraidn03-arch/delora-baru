...
- (2026-09-18) Overlay modal/drawer FINAL sesuai referensi user: dim gelap klasik `bg-black/60` TANPA blur — modal putih tajam di tengah. Customer: kolom `address` (migrasi via Management API sukses), field Alamat di form + kolom tabel + card mobile. Quick View: tombol "Kirim ke WhatsApp Customer" (wa.me + nomor dari data customer, auto-normalize 08→62, toast panduan bila nomor kosong), pesan invoice lebih profesional ("Kepada Yth.", rincian, "Pembayaran dapat dilakukan melalui transfer ke" 🏦 BRI a.n. I Nyoman Tri Adnyana 800601014225533 & 🏦 BCA a.n. I Nyoman Tri Adnyana 3950716265, instruksi bukti transfer, ucapan terima kasih) — footer PDF invoice disamakan.
- (2026-09-18) FIX bug modal terpotong + overlay: Modal.tsx kini dirender via React Portal ke document.body (menetralkan containing-block dari transform .animate-fade-up), container scrollable tanpa overlay sama sekali (transparan), panel shadow besar + border, mobile tetap full-screen. TERVERIFIKASI testing agent iterasi 4: 100% pass — header tidak terpotong di semua modal, outside-click/Escape/X menutup, quickview panjang scrollable, mobile 390px full-screen.
